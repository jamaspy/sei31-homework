...
def plan_trip (line_on, stop_on, line_end, stop_end)
  #The program takes the line and stop that a user is getting on at
  line_on = ''
  stop_on = ''
  #change line
  change = 'Union Square'
  #the line and stop that user is getting off at
  line_end = ''
  stop_end = ''
  ...
  #new arrays with the stops
  journey = #all stops
  new_journey = []#if diferent lines

  #logic
  if line_on == line_end 
    print "#{line_on} #{stop_on} #{journey} " 
  else 
    print "#{line_on} #{stop_on} #{journey} "
    print "Change at #{change}"
    print "#{line_off} #{new_journey} #{stop_off}"
  end
