...
It is popular to sing on long trips, as it is repetitive and easy to memorise but -- it 
can take a long time to sing.
...
const singBottles = function(num) {
  
  for (var i = 1; i <= num; i +=1) {
    let result = num -i;
    //console.log(result);
    //let song2 = `${result} bottles of beer on the wall, ${result} bottles of beer.`;
    let song = `${num} bottles of beer on the wall, ${num} bottles of beer.`;

    if (typeof(num) === 'number') {
      console.log(song);
    } else if (result === 0) {
      console.log(result);
      console.log(`${result} bottles of beer on the wall, ${result} bottles of beer.`)
    }
  }
};
